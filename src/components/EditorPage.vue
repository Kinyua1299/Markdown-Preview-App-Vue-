...
import CodeMirror from '@uiw/vue-codemirror'
...
const markdownText = ref<string>(() => {
  try {
    return localStorage.getItem('md_content') || DEFAULT_MARKDOWN
  } catch {
...
const isDark = ref<boolean>(() => {
  try {
    return localStorage.getItem('md_theme') === 'dark'
  } catch {
    return true
  }
})()
...
const autoScroll = ref<boolean>(() => {
  try {
    return localStorage.getItem('md_auto_scroll') === 'true'
  } catch {
    return false
  }
})()
...
async function handleFileLoad(e: Event) {
  ...
  try {
    ...
    previewRef.value && (previewRef.value.scrollTop = 0)
...
watch([markdownText, autoScroll], async () => {
  ...
  await nextTick()
  const pr = previewRef.value
  pr.scrollTop = (pr.scrollHeight - pr.clientHeight) * scrollState.lastRatio
})
    ...
    <div class="mx-auto w-full max-w-7xl flex-grow flex flex-col overflow-hidden">
      <nav class="flex flex-col sm:flex-row justify-between gap-2 p-4 border-b">
        ...
        <div class="flex flex-wrap gap-2 items-center">
          ...
          <button
            class="btn"
            @click="markdownText = DEFAULT_MARKDOWN"
          >
            Reset
          </button>
      ...
      <div class="flex flex-col md:flex-row flex-1 overflow-hidden p-4 gap-4">
        ...
        <div
          v-if="previewVisible"
          ref="previewRef"
          class="md:w-1/2 h-[50vh] md:h-full overflow-auto rounded border p-4"
          v-html="renderedHTML"
        />
      </div>
    </div>
